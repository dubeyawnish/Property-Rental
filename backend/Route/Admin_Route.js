...
router.post('/listProperty', (req, res) => {
  ...
  const { ownerName, mobileNumbers, emailAddresses, villaApartmentNumber, projectName,builderName, location,showLocation, yearOfCompletion,imgUrl,projectImg,propertyType, expectedRentPrice, expectedSalePrice, monthlyMaintenance, builtUpArea,carpetArea, plotSize,propertyImgName, bedrooms, bathrooms, balconies, carParks,latitude,longitude, mainDoorDirection, studyRoom, maidsRoom, maidsToilet, privatePool, privateGarden, privateTerrace, homeTheatreRoom, mediaRoom, modularKitchen, airConditioner, bed, chimney, curtains, diningTable, dishwasher, dryer, geyser, hob, mattress, microwave, oven, refrigerator, sofaSet, solarHeater, tv, wardrobe, washingMachine, waterPurifier, granite, italianMarble, kotaStone, marble, tiles, wood, additionalInformation } = req.body;


  const property = new AdminProperty({ ownerName, mobileNumbers, emailAddresses, villaApartmentNumber, projectName,builderName, location,showLocation, yearOfCompletion,imgUrl ,propertyType,projectImg, expectedRentPrice, expectedSalePrice, monthlyMaintenance, builtUpArea,carpetArea, plotSize,propertyImgName,bedrooms, latitude,longitude,  bathrooms, balconies, carParks, mainDoorDirection, studyRoom, maidsRoom, maidsToilet, privatePool, privateGarden, privateTerrace, homeTheatreRoom, mediaRoom, modularKitchen, airConditioner, bed, chimney, curtains, diningTable, dishwasher, dryer, geyser, hob, mattress, microwave, oven, refrigerator, sofaSet, solarHeater, tv, wardrobe, washingMachine, waterPurifier, granite, italianMarble, kotaStone, marble, tiles, wood, additionalInformation });
...
router.get('/getAllLocation', async (req, res) => {
  try {
    const allLocation = await AdminProperty.find({}, 'showLocation propertyType');
    const uniqueProjects = [];
    const projectNames = [];
